...
local library, but other times is has to be transferred from another
...
\item Data associated with the program, including static data (allocated
  at compile time) and dynamic data including the run-time stack and
  the heap.
...
\item The text segment contains the program text; that is, the
  machine language instructions that make up the program.

\item The static segment contains variables that are allocated
  by the compiler, including global variables and local variables
  that are declared {\tt static}.

\item The stack segment contains the run-time stack, which is
  made up of stack frames.  Each stack frame contains the
...
\item The static segment is often just above the text segment.
...
static segment.  {\tt local} is a local variable, so we expect it
to be on the stack.

And {\tt p} contains an address returned by {\tt malloc}, which allocates
space in the heap.  ``malloc'' stands for ``memory allocate.''

The format sequence \verb"%p" tells {\tt printf} to format each
address as a ``pointer'', which is another name for an address.
...
As expected, the address of {\tt main} is the lowest, followed
by {\tt global} and {\tt p}.  The address of {\tt local} is
much bigger.  It has 12 hexadecimal digits.  Each hex digit
...
\item The MMU looks up the page number in the ``page table'' and gets
the corresponding physical page number.  Then it combines
...
As an example, suppose that the VA is 32 bits and the physical memory
is 1 GiB, divided into 1 KiB pages.
...
Fortunately, nothing like that much space is actually needed, because
...
systems, including Linux, use.  Another
option is an associative table, where each entry includes both
the virtual page number and the physical page number.  Searching
an associative table can be slow in software, but in hardware we
can search the entire table in parallel, so associative arrays are
often used to represent pages tables in the MMU.
...
time to read a block from disk to memory is typically 2--6 ms.
...
so while waiting for a byte from an SSD, it would complete 50,000.

In one millisecond, it would complete 2,000,000 instructions,
so while waiting for a byte from a HDD, it might complete
10 million.  If there's nothing for the CPU to do while it waits,
...
\item Block transfers: The time it takes to load a single byte from
  disk is about 5 ms.  By comparison, the additional time to load an 8
  KiB block is negligible.  If the processor does 5 ms of work on
  each block, it might be possible to keep the processor busy.
...
information about the file including: the user ID of the file owner,
...
but it's obviously not big enough for all of them.  That's
...
``workload characteristics'' including file sizes, access
...
\[ (-1)^s c \cdot 2^q \]
%
Well, that's almost correct, but there is one more wrinkle.
...
Well, that's almost correct.  There's one more wrinkle.
...
is by shifting 1
to the left by 23 places and then subtracting 1.
...
Also, remember that the letters {\it and numbers} in C strings are
...
is, sets all bytes in the chunk to 0.

\item {\tt free}, which takes a pointer to a previously allocated
chunk and deallocates it; that is, makes the space available for
future allocation.
...
which is why most modern language provide higher-level memory
...
who appears as a strict substitute teacher who imposes corporal
punishment, a ``paddlin''', for all infractions.

Here are some of things a program can do that deserve a paddling:

\begin{itemize}

\item If you access any chunk that has not been
...
of the documented interface, information about how that memory should
be freed.  Mature libraries often do this well, but in the real world,
...
\item If you read a value from an unallocated chunk, the system {\em
  might} detect the error, trigger a run-time error called a
  ``segmentation fault'', and stop the program.  This outcome is
  desirable, because it indicates the location in the program that
  caused the error.  But, sadly, this outcome is {\em rare}.  More
  often, the program reads unallocated memory without detecting the
  error, and the value it gets is whatever happened to be stored at a
  particular location.  If the value is not interpreted as the right
  type, the results might be hard to interpret.  For example, if you
  read bytes from a string and interpret them as a floating-point
  value, you might get an invalid number or a number that is
  extremely large or small.  If you pass that value to a function that
  isn't prepared to handle it, the results can be bizarre.

\item If you write a value to an unallocated chunk, things are even
  worse, because after the bad value is written, a long time might
...
Often there is a tradeoff between safe memory management and performance.
...
the process terminates.  By convention, status code 0 indicates normal
...
harder to read.  But you can mitigate the problem by wrapping library
...
When {\tt sbrk} is called, it allocates new pages of physical
...
To implement the memory management API,
...
\item Boundary tags: When {\tt malloc} allocates a chunk, it adds
  ...
  list, so each chunk contains pointers to the next and previous
...
\item Binning and caching: The free list is sorted by size into bins,
  ...
  knows what bin to search in.  So if you free a chunk and then
...
that it initiates a new instruction every 0.5 ns.  But the time it
takes to transfer data to and from memory is about 10 ns.  If the CPU
has to wait 10 ns to fetch the next instruction, and another 10 ns to
load data, it would take 40 clock cycles to complete one instruction.


\section{Cache performance}

The solution to this problem, or at least a partial solution, is
caching.  A ``cache'' is a small, fast memory on the same chip
as the CPU.  In current computers, a cache might be 1--2 MiB, and
the access time might be 1--2 ns.
...
of the cache.  If the data needed by the CPU are usually in cache,
the program can run at the full speed of the CPU.  If the CPU
...
after that the rest of the string would be in cache.  After
reading the whole string, the hit rate would be 63/64.
If the string spans two blocks, you would incur 2 miss penalties.  But
even then the hit rate would be 62/64, or almost 97\%.
...
penalty is about 9 ns.  We can infer that the block size is 128 B.
...
Also main memory is usally packaged in a dual in-line memory modules
(DIMM) that includes 16 or more chips.  Several small chips are cheaper
than one big one.

The tradeoff between speed, size, and cost is the fundamental reason
...
are fast, but they are more expensive than hard drives, so they tend to
be smaller.  Tape drives are even slower than hard
...
\item What gets moved?  In general, block sizes are small at the top
  ...
  when the operating system reads a file from disk, it might read 10
  or 100 blocks at a time.
...
\item When does data get moved?  In the most basic cache, data gets
  ...
  loaded before it is explicitly requested.  We have already seen a
  simple form of prefetching: loading an entire block when only part of
  it is requested.
...
\item Process A calls {\tt malloc} to allocate a chunk.  If there
...
page'' belonging to Process B.  It copies the contents of the
...
additional space in the heap.  Then {\tt malloc} allocates the request
chunk and returns.  Process A can resume.

\item When Process A completes, or is interrupted, the scheduler might
allow Process B to resume.  When it accesses a page that has been swapped
...
\item Also, there might be many processes running the same program.
  These processes can share the same text segment, avoiding the need
...
This regrettable scenario is called ``thrashing''.
...
A hardware interrupt is caused when a device sends a signal to the
...
A software interrupt is caused by a running program.  For example, if
...
Some floating-point errors, like division by zero, can be handled
using interrupts.

When a program needs to access a hardware device,
it makes a ``system call'', which is similar to a function call,
...
instructions running on the CPU and data stored in registers.
...
to save the contents of the registers.  In order to do its job, the
kernel needs to execute instructions, which modify data registers and
the flag register.  So this ``hardware state'' needs to be saved
before it gets modified, and then restored before the interrupted
process resumes.
...
flag register in memory, along with the contents of any data
...
the interrupted process to know there was an interruption, unless
it detects small changes in the time between instructions.
...
unit (see
Section~\ref{address_transaltion}).  And after the context switch, it
...
Here are the events that cause a process to transition from
...
counter and other hardware state, and the run-time stack.
...
interfering with each other.  And usually threads can't access each
...
The most popular threading standard used with C is
